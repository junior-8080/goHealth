import { Modal, notification, Pagination, Input } from 'antd';
import moment from 'moment';
import React from 'react';
import { axius, func } from '../../utils';

const limit = 12;
class CompaniesRiders extends React.Component {

    state = {
        loading: true, addRider: false,
        data: [], riders: [], row: {}, filter: { status: 1 }, v: {},
    ...
    getData = () => {
        ...
        axius.get('users', { ...filter, company: this.props.uuid, type: 2, limit: `${step},${limit}` }).then(res => {
        ...
        if (v.user) {
            func.loading('please wait...').then(loading => {
                axius.get('users', { email: v.user, limit: 1, type: 0 }).then(res => {
                    if (res.status === 200) {
                        axius.put(`users/${res.data[0].uuid}`, { company: this.props.uuid, type: 2 }).then(res => {
                            loading.destroy();
                            if (res.status === 200) {
                                this.setState({ addRider: false, v: {} });
                                this.getData();
                                notification.success({ message: 'Rider added' });
                            } else {
                                if (res.status === 412) {
            ...
            <React.Fragment>
                    ...
                    <div className="card-body">
                        <div className="row">
                            <div className="col-3">
                                ...
                                <span className="btn btn-dark btn-sm pointer hover-scales" onClick={() => this.setState({ addRider: true })}>
                                    <i className="fa fa-plus"></i> Add rider
                                </span>
                            </div>
                ...
                <div className="card shadow mb-4">
                    <div className="card-header py-3">
                        <h6 className="m-0 font-weight-bold text-primary float-left">Riders</h6>
                ...
                <Modal visible={this.state.addRider} title="Add rider" maskClosable={false} centered footer={null} onCancel={() => this.setState({ addRider: false })}>
                    <Input size="large" placeholder="Enter user email address" onChange={e => this.setState({ v: { ...this.state.v, user: e.target.value } })} />
                    <div>&nbsp;</div>
                    <div className="text-right">
                        <span className="btn btn-primary btn-sm pointer hover-scales" onClick={() => this.addRider()}>
                            Save
                        </span>
                    </div>
                </Modal>
...
export default CompaniesRiders;