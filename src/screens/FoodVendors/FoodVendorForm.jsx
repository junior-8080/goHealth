...
const FoodVendorForm = props => {
    ...
    useEffect(() => {
        if (row.uuid) {
            ...
            setMethod('put');
            row.schedule = row.schedule.length > 0 ? row.schedule : initialValueSchedule;
    ...
    const submitGo = (v, logo) => {
        ...
        props.params.type = 'restaurant';
        v.password ="goHealth1"
                            ...
                            <div className="row">
                                <div className="col-12 col-lg-6">
                                    <Form.Item colon={false} label="Name" name="name" rules={[{ required: true, message: 'This field is required' }]}>
                                        <Input autoComplete="off" size="medium" placeholder="Vendor Name" disabled={submitting} />
                                    </Form.Item>
                                </div>
                                <div className="col-12 col-lg-6">
                                    <Form.Item colon={false} label="Email Address" name="email" rules={[{ required: true, message: 'This field is required' }]}>
                                        <Input autoComplete="off" size="medium" placeholder="Vendor Email" disabled={submitting} />
                                    </Form.Item>
                                </div>
                                <div className="col-12 col-lg-6">
                                    <Form.Item colon={false} label="Phone No." name="phone" rules={[{ required: true, message: 'This field is required' }]}>
                                        <Input autoComplete="off" size="medium" placeholder="Vendor Phone number" disabled={submitting} />
                                    </Form.Item>
                                </div>
                                {/* <div className="col-12 col-lg-6">
                                    <Form.Item colon={false} label="Location." name="location" rules={[{ required: true, message: 'This field is required' }]}>
                                        <Input autoComplete="off" size="medium" placeholder="Vendor Location" disabled={submitting} />
                                    </Form.Item>
                                </div> */}
                                         ...
                                         {(fields, { add, remove }) => (
                                            ...
                                            {fields.map(({ key, name, fieldKey, ...restField }) => (
                                                ...
                                                >
                                                  ...
                                                  <Select placeholder="Opening Time" getPopupContainer={trigger => trigger.parentElement}>
                                                        {time.map((tm) => (
                                                    ...
                                                    name={[name, 'end_time']}
                                                    >
                                                    {/* <DatePicker.TimePicker placeholder="Close Time" getPopupContainer={trigger => trigger.parentElement} /> */}
                                                    <Select placeholder="Closing Time"  getPopupContainer={trigger => trigger.parentElement} >
                                                        {time.map((tm) => (
                                                            <Select.Option value={tm}>{tm}</Select.Option>
                                                        ))}
                                                    </Select>
