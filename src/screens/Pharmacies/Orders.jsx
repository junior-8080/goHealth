...
import { orders } from '../../utils/timer';
            ...
            <React.Fragment>
                        ...
                        <div className="row" style={{display:"flex",justifyContent:"space-between"}}>
                            <div className="col-3">
                                <Select size="default" showSearch value={filter.status} onChange={status => this.setState({ filter: { ...filter, status } }, () => { this.getData() })}>
                                    ...
                                    {settings.order_status.map((s) => (
                                        <Select.Option key={s.id} value={i.id}>{s.name}</Select.Option>
                                    ))}
                                </Select>
                            </div>
                           {this.props.params !== 'pharmacies' ? <div className="col-3">
                                <Select size="default" showSearch value={filter.status}>
                                    <Select.Option value="">All Orders</Select.Option>
                                    {orders.map((order,i) => (
                                        <Select.Option key={order.value + i} value={order.value}>{order.name}</Select.Option>
                                    ))} 
                                </Select>
                            </div> :" "}
                ...
                <div className="card shadow mb-4">
                    <div className="card-header py-3">
                        <h6 className="m-0 font-weight-bold text-primary float-left">Orders</h6>
